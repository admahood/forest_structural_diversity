...
dd |>
  ...
  ggplot(aes(x=nspp_exotic, y = nspp_notexotic)) +
  geom_point() +
  facet_wrap(~ent_class) +
  geom_smooth(method = 'lm')
...
resp <- c("rel_cover_exotic", "shannon_native","rer", "nspp_native")
...
topranks <-
  ...
  filter(toprank == "top5")%>%
  mutate(response = ifelse(response == "shannon_native_all", "A. Native Alpha Diversity", response),
         response = ifelse(response == "nspp_native_all", "B. Native Richness", response),
         response = ifelse(response == "rer_all", "C. Exotic Relative Richness", response),
         response = ifelse(response == "rel_cover_exotic_all", "D. Exotic Relative Cover", response),
         variable = variable %>% str_remove_all(".aop") %>%str_remove_all(".AOP"))


barplot <- dfs %>%
  bind_rows() %>%
  filter(response %in% c("rel_cover_exotic_all", "rer_all", 
